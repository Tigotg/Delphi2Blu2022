...
var
  xPercImposto, xPercDistrib, xPrecoFinal, xValorCompra: Double;
begin
  xPercImposto := StrToFloat(edtPercImposto.Text);
  xPercDistrib := StrToFloat(edtPercDistrib.Text);

  xValorCompra := StrToFloat(edtValorCompra.Text) * (xPercImposto / 100 + 1);
  xPrecoFinal := xValorCompra * (xPercDistrib / 100 +1) ;
