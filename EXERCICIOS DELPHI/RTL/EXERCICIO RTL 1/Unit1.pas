...
begin
  ...
  try
    if buttonSelected1 = mrYes then
      raise Exception.Create('Presssione outro botão');
  except
    on E: Exception do
    begin
      Raise;
...
begin
  ShowMessage('[' + TrimLeft(PaddedString)  + ']'); //Remove os espaços da esquerda
  ShowMessage('[' + TrimRight(PaddedString) + ']'); //Remove os espaços da Direita
